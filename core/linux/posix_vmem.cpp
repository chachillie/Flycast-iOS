...
{
#ifdef TARGET_IPHONE
    // Try TXM mode first (iOS 26 with StikDebug)
    init_txm_jit_pool();
    ...
    if (g_txm_pool.uses_txm)
    {
        ...
        *code_area_rwx = (u8*)aligned - g_txm_pool.rw_rx_diff;
        ...
        DEBUG_LOG(VMEM, "TXM alloc: RW=%p RX=%p size=%zu",
    ...
    // Original code path for non-TXM devices
    ...
    if (code_area != nullptr)
    {
        ...
        ret_ptr = mmap(code_area, size, PROT_READ | PROT_WRITE | PROT_EXEC,
                      MAP_FIXED | MAP_PRIVATE | MAP_ANON, 0, 0);
    }
    if (ret_ptr == MAP_FAILED)
    {
        ret_ptr = mmap(nullptr, size, PROT_READ | PROT_WRITE | PROT_EXEC,
                      MAP_PRIVATE | MAP_ANON, -1, 0);
        if (ret_ptr == MAP_FAILED)
            return false;
    }
#else
    // MAP_JIT for Apple Silicon Mac
    void *ret_ptr = mmap(NULL, size, PROT_READ | PROT_WRITE | PROT_EXEC,
...
bool prepare_jit_block(void *code_area, size_t size, void **code_area_rw, ptrdiff_t *rx_offset)
{
    ...
    *rx_offset = (char*)ptr_rx - (char*)ptr_rw;
    ...
    INFO_LOG(DYNAREC, "Using NO_RWX mode, rx ptr: %p, rw ptr: %p, offset: %ld",
             ptr_rx, ptr_rw, (long)*rx_offset);
